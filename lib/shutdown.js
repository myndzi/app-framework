...
module.exports = function (app) {
    ...
    app.restart = function (msg, timeout) {
        return app.shutdown(EXIT_CODES.RELOAD, msg, timeout);
    };
    app.shutdown = function (/*[code], [msg], [timeout]*/) {
        // jshint maxstatements: 25, maxcomplexity: 8
        if (app._shutdownPromise) {
        ...
        if (timeout === null) {
            timeout = app.config.get('app.shutdownTimeout') || null;
        }
        ...
        function makePromise(arg) {
            if (typeof arg.once === 'function') {
                return new Promise(function (resolve, reject) {
                    arg.once('error', reject);
                    arg.once('end', resolve);
                    arg.once('close', resolve);
                });
            }
        ...
        var cleanupHandlers = Promise.all(
