...
// must be loaded before other modules -- hooks instrumented modules via 'require'
if (process.env.ENABLE_TRACEVIEW) {
...
module.exports = function (config) { // jshint ignore: line
    ...
    if (!config.log.hasOwnProperty('level')) {
        // config files may/will change this later, this mostly just affects logging
        // before the configuration is fully loaded
        config.log.level = (config.env === 'development' ? 'info' : 'warn');
    }
    ...
    app.on('loaded', function () {
        app.log.debug('Resolved config:', app.config.get());
    });
